...
#############################
# crime
crime<-read.csv('raw data/crime data/crimeOnzip.Trulia.csv')
...
vegas@data<-merge(vegas@data,crime,all.x=TRUE,by.x='ZCTA5CE10',by.y='ZCTA5CE10')
vegas@data<-vegas@data[order(vegas@data$order),]# back to original order 
...
vegas$Crime_SQMI<-vegas$Total.Crime2016/vegas$ALAND_SQMI #crime: per sql mile #2016
